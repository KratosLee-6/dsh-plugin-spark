...
- **既能给 Agent 用，也能自己体验。** 七个 DSH 原生工具、双语工作室、SKILL.md 导出。碰撞动画可取消，尊重减少动态效果设置，导航没有音效。
...
内置四个原创合成示例。可参照[技能契约示例](examples/skill.json)导入自己的 JSON。已有 ID 不允许静默覆盖：相同内容可重复导入，修改内容请使用新 ID。SKILL.md 可选择 Markdown 格式，先预览，再补全契约并确认保存。未知内容明确展示，不会自动写入字段。详见[支持语法与导入指南](docs/IMPORT.md)。
...
打开本地 Markdown 文件（最大 64 KB），检查映射结果，逐项编辑契约后再保存。修改原文会使先前确认失效；预览本身不写入资料库。Spark 导出的草案再导入时可保留标签与父级编号。

![中文 Markdown 契约审阅](docs/screenshots/import-review-zh.png)

在 DSH 中先用 `spark_preview_import` 的 `markdown` 参数预览，向用户展示草案及未映射内容；用户要求保存后，再将核对后的 `skill_json` 交给 `spark_import`。预览不代表安全认证，也不证明技能已能执行。

## 七个工具

| 工具                   | 能力                                           | 是否写入        |
| ---------------------- | ---------------------------------------------- | --------------- |
| `spark_preview_import` | 预览 SKILL.md、未映射内容和缺失契约            | 否              |
...
后续候选：显式类型转换适配、工作区备份恢复、带来源依据的可选模型建议、DSH 原生结果卡片。这些是待评估方向，不是已交付功能。
